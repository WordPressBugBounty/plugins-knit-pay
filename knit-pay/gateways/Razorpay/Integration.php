...
class Integration extends IntegrationOAuthClient {
	...
	public function __construct( $args = [] ) {
		...
		// Serve the Apple Pay domain verification file via WordPress (works on Apache and nginx).
		add_action( 'template_redirect', [ $this, 'serve_apple_pay_domain_verification' ] );
	}
	...
	/**
	 * Serve the Apple Pay domain verification file via WordPress.
	 *
	 * Intercepts requests to /.well-known/apple-developer-merchantid-domain-association
	 * and outputs the file contents directly. This approach works on both Apache
	 * and nginx without any server configuration changes.
	 */
	public function serve_apple_pay_domain_verification() {
		...
		if ( ! str_contains( $request_uri, '/.well-known/apple-developer-merchantid-domain-association' ) ) {
			return;
		}
