...
class Gateway extends Core_Gateway {
	...
	public function redirect_via_html( Payment $payment ) {
		...
		\wp_register_style(
			...
			KNITPAY_URL . "/gateways/UpiQR/css/template{$this->config->payment_template}.css",
			[ 'dashicons' ],
			KNITPAY_VERSION
		);
	...
	public function enqueue_scripts() {
		\wp_register_script(
			...
			KNITPAY_URL . '/gateways/UpiQR/js/easy.qrcode.min.js',
			[],
			'4.6.2',
			[ 'in_footer' => true ]
		);
		...
		\wp_register_script(
			'knit-pay-upi-common',
			KNITPAY_URL . '/gateways/UpiQR/js/upi-common.js',
			[ 'knit-pay-easy-qrcode' ],
			KNITPAY_VERSION,
			[ 'in_footer' => true ]
		);
		...
		\wp_register_script(
			...
			KNITPAY_URL . '/gateways/UpiQR/js/countdown-timer.js',
			[],
			KNITPAY_VERSION,
			[ 'in_footer' => true ]
		);
		...
		\wp_register_script(
			"knit-pay-upi-qr-template-{$this->config->payment_template}",
			KNITPAY_URL . "/gateways/UpiQR/js/template{$this->config->payment_template}.js",
		...
		wp_localize_script(
			...
			[
				...
				'payment_status_worker_url' => KNITPAY_URL . '/gateways/UpiQR/js/payment-status-worker.js',
			]
		);
	}
}
