...
class Client {
	...
	private function authenticate() {
		...
		$response = wp_safe_remote_post(
			...
			[
				...
				'timeout' => 30, // phpcs:ignore WordPressVIPMinimum.Performance.RemoteRequestTimeout.timeout_timeout -- Pesapal OAuth token endpoint can be slow; 30s ensures reliability.
			]
		);

		if ( is_wp_error( $response ) ) {
			throw new Exception( esc_html( 'Authentication failed: ' . $response->get_error_message() ) );
		}
	...
	public function create_payment( array $data ) {
		...
		$response = wp_safe_remote_post(
			...
			[
				...
				'timeout' => 30, // phpcs:ignore WordPressVIPMinimum.Performance.RemoteRequestTimeout.timeout_timeout -- Pesapal payment submission can be slow; 30s ensures reliability.
			]
		);
		...
		if ( isset( $result->error ) ) {
			throw new Exception( esc_html( $result->error->message ?? 'Payment creation failed' ) );
		}
	...
	public function get_payment_status( string $order_tracking_id ) {
		...
		$endpoint = $this->api_url . '/Transactions/GetTransactionStatus?orderTrackingId=' . rawurlencode( $order_tracking_id );

		$response = wp_safe_remote_get(
			$endpoint,
			[
				'headers' => $this->get_request_headers(),
				'timeout' => 30, // phpcs:ignore WordPressVIPMinimum.Performance.RemoteRequestTimeout.timeout_timeout -- Pesapal transaction status check needs adequate timeout.
			]
		);

		if ( is_wp_error( $response ) ) {
			throw new Exception( esc_html( 'Failed to get payment status: ' . $response->get_error_message() ) );
		}
	...
	public function register_ipn( string $notification_url ) {
		...
		$response = wp_safe_remote_post(
			...
			[
				...
				'headers' => $this->get_request_headers(),
				'timeout' => 30, // phpcs:ignore WordPressVIPMinimum.Performance.RemoteRequestTimeout.timeout_timeout -- Pesapal IPN registration can be slow; 30s ensures reliability.
			]
		);

		if ( is_wp_error( $response ) ) {
			throw new Exception( esc_html( 'IPN registration failed: ' . $response->get_error_message() ) );
		}
